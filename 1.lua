...
-- Try to stop Camera Shake (wrapped in pcall to avoid errors)
pcall(function()
...
function MoonTextureId()
    if Sea1 or Sea2 then
        return pcall(function() return game:GetService("Lighting").FantasySky.MoonTextureId end)
    elseif Sea3 then
        return pcall(function() return game:GetService("Lighting").Sky.MoonTextureId end)
    end
    return false, nil
end
...
function CheckMoon()
    ...
    local success, moonreal = MoonTextureId()
    local moonStatus = "Bad Moon"
    
    if success and moonreal then
...
function GetGameTime()
    local success, clockTime = pcall(function() return game.Lighting.ClockTime end)
...
function GetFormattedTime()
    local success, clockTime = pcall(function() return game.Lighting.ClockTime end)
    if not success then return "00:00" end
...
local function fetchFullMoonServers()
    ...
    if not success then
        warn("Failed to fetch Full Moon servers: " .. tostring(response))
        return {}
    end
    ...
    if not success then
        warn("Failed to parse API response")
        return {}
    end
...
-- Create Status Screen (wrapped in pcall to avoid errors)
local StatusScreen, StatusFrame, StatusLabel
pcall(function()
    StatusScreen = Instance.new("ScreenGui")
    ...
    if syn and syn.protect_gui then
        syn.protect_gui(StatusScreen)
        StatusScreen.Parent = game.CoreGui
    elseif gethui then
        StatusScreen.Parent = gethui()
    else
        StatusScreen.Parent = game.CoreGui
    end
